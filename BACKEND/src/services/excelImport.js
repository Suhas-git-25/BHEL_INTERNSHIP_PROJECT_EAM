...
async function resolveIssuedBy(conn, issuedBy) {
  const userId = Number(issuedBy);
  if (!userId) {
    return null;
  }

  const r = await conn.execute(`SELECT user_id FROM eam_app_user WHERE user_id = :userId`, {
    userId,
  });
  return rowsToClients(r)[0] ? userId : null;
}
...
async function importIssues(conn, rows, bucket, issuedBy) {
  for (const row of rows) {
    ...
    try {
      await conn.execute("SAVEPOINT import_issue_row");
      ...
      if (!holderId) {
        ...
        bucket.skipped += 1;
        await conn.execute("RELEASE SAVEPOINT import_issue_row");
        continue;
      }
      ...
      try {
        await conn.execute("ROLLBACK TO SAVEPOINT import_issue_row");
        await conn.execute("RELEASE SAVEPOINT import_issue_row");
      } catch {
        /* Nothing to roll back if the row failed before the savepoint opened. */
      }
      addError(bucket, row.__row, err.message);
      bucket.skipped += 1;
    }
...
async function importCategoryBuffer(buffer, category, issuedBy) {
  ...
  await withConnection(async (conn) => {
    ...
    if (category === "issues") {
      const validIssuedBy = await resolveIssuedBy(conn, issuedBy);
      await handler(conn, rows, summary, validIssuedBy);
    } else {
